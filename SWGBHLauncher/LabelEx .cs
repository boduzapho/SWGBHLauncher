...
namespace SWGBHLauncher
{
    class MoreRealisticShadowLabel : Label
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            Bitmap shadowBitmap = new Bitmap(Math.Max((int)(Width / softness), 1),Math.Max((int)(Height / softness), 1));
            using (Graphics imageGraphics = Graphics.FromImage(shadowBitmap))
            {
                ...
                transformMatrix.Translate((float)(shadowDepth * Math.Cos(direction)),(float)(shadowDepth * Math.Sin(direction)));
